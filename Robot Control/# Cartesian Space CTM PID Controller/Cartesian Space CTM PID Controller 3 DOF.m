...
init_q3  = pi/2;        % [rad]    : Initial Joint Angle
...
init_X = get_Kinematics2(q(1), q(2), q(3)); % [m] : Init End-Effector Position

X     = init_X;                      % [m] : Current Position
dX    = [0; 0; 0];                      % [m] : Current Velocity
X_d   = init_X;                      % [m] : Target End-Effector Position
dX_d  = [0; 0; 0];                      % [m] : Target End-Effector Velocity
ddX_d = [0; 0; 0];                      % [m] : Target End-Effector Acceleration

% Contoller Gain

Wn    = 20; % 20 -> 60 good -> 120 better
Kp    = Wn^2;              % P Gain
Kv    = 2*Wn;              % D Gain
...
for time = start_t:delta_t:finish_t
    % Robot Model
        ...
        elseif time < 2.0
            X_d(1) = init_X(1);                    % x , z 위치는 보정 불필요

            if X_d(2) < init_X(2) + 0.2              % 1m 만큼 올라가기 때문에 1 보정
                X_d(2) = X_d(2) + (0.2/0.5)*delta_t; % (0.2/0.5)*delta_t : Target Velocity 0.2의 거리를 0.5초 만에 이동 --- 즉, 0.4 m/s X 적분 --- 0.4 * dt --- 위치 보정 !
            else
                X_d(2) = init_X(2) + 0.2;            % 목표 위치에 도달한다면 멈춰라
            end
            ...
            X_d   = [0.2 * sin((2*pi*sin_t)) + init_X(1);        % 초기 X위치 : 0.1 + Initial X
                     0.2 * cos((2*pi*sin_t)) + init_X(2);
                     0];      % 초기 Y위치 : 0.2 + Initial Y
            sin_t = sin_t + delta_t;
        ...
        % Dynamics
        J     = get_Jacobian2(q(1), q(2), q(3));     % 자코비안을 구해오는 코드 구현
        dJ    = (J - pre_J)/delta_t;          % Jacobian 미분
        pre_J = J;                            % 이전 Jacobian
        X     = get_Kinematics2(q(1), q(2), q(3));   % Current Position
        dX    = J*dq;                         % Velocity
        ...
        u = ddX_d + Kv*(dX_d - dX) + Kp*(X_d- X) + Ki*Integral_temp;
        ddq_ref = pinv(J)*(u - dJ*dq); % 2 x 1
...
title('Cartesian 3 DOF PID CTM Controller', 'fontsize', font_size_title)
