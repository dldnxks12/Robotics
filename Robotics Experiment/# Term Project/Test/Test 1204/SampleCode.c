...
// 제어 게인 튜닝 ~ing 
volatile double pos_P = 0.1;
volatile double pos_D = 0.1;
volatile double vel_P = 0.1;
volatile double vel_I = 0.1;

volatile double cur_P = 0.1;
...
ISR(TIMER0_OVF_vect){ 
	...
	{	  		
		...
		g_Perr_dif = (g_Perr - g_Ppre_err) / 0.1;
		g_Ppre_err = g_Perr;
	...
	{				
		g_Vcur = (g_Pcur - g_Ppre) / 0.01;
		g_Ppre = g_Pcur;
		g_Vdes = g_pos_control;
		g_Verr = g_Vdes - g_Vcur;
		
		g_Verr_sum += g_Verr; // 속도 적분값
		
		if(g_Verr_sum < -(200/57.2958))
		{
			g_Verr_sum = -(200/57.2958);
		}
	...
	if(g_TimerCnt % 10 == 0){ // 제어 주기 : 0.001초 				
		...
		if(g_Cerr_sum <= -(1200 / 57.2958) )
		{
			g_Cerr_sum =  -5000 / 57.2958;
		}
