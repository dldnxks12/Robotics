...
// 16MIPS 
#define F_CPU 16000000UL // 16 Mhz : 초당 1600만개 명령 수행 
...
void InitIO();     // PORT 설정
void InitExtInt(); // 외부 인터럽트 설정

// Timer 
void InitTimer0(); // 0 Timer 설정 - Normal Count , OCIE 
void InitTimer1(); // 1 Timer 설정 - PC PWM
void InitTimer2(); // 2 Timer 설정 - None 
void InitTimer3(); // 3 Timer 설정 - PC PWM

// Analog -> Digital 
void InitADC();  // ADC 설정 - 0번 채널 
int GetADC(char ch); // 변환된 AD Value 가져오기 

// UART 통신 
void InitUart0(); // USART 0 Setting <- 일단 얘 사용 
void InitUart1(); // USART 1 Setting 

void TransUart0(unsigned char data); // data 보내기 
void TransUart1(unsigned char data); // data 보내기 

unsigned char RecvUart0(); // data 받기
unsigned char RecvUart1(); // data 받기
