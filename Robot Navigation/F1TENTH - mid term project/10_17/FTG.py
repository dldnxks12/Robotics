# Speed Filter --- 1. jittering 잡아낼 수 있는지 2. 시간차이가 얼마나 나는지
...
class CustomDriver:
    ...
    def Sudden_gap(self, distance):
        before = self.BEFORE_DISTANCE
        gap = current - before
    ...
    def process_lidar(self, ranges):
        ...
        if self.BEFORE_DISTANCE != 0:
            GAP = self.Sudden_gap(distance)
            if GAP > 2:
        ...
        speed = (-a * abs(steering_angle)) + (self.b * distance) + (c * self.TIME) + (-d * LINE) + (self.e) + 6

        #speed = self.speed_filter(speed)

        print(
            f'B value : {self.b : .3f} | S : {speed : .3f} | D : {distance : .3f} | T : {self.TIME} T_ : {(c * self.TIME) : .3f}',
            end='\r')
